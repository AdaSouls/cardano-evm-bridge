...
const aldeaBridgeEvent = catchAsync(async (req, res) => {
  try {
    ...
    if (webhookEvent.type === "transaction") {
      ...
      console.log("--------");
      console.log(JSON.stringify(txMetadata.data, null, 2));
      console.log("--------");

      if(txMetadata.data[0].label === '674') {
        ...
        switch(txMetadata.data[0].json_metadata.network) { // chain      
          case "pyrope":
            ...
            if(address(txMetadata.data[0].json_metadata.destination)) {
              ...
              console.log(`Address ${txMetadata.data[0].json_metadata.destination} is not valid`);
              errorService.internalError(res);
              res.status(200).send();
              return;
            }
            break;
          default:
            console.log(`Chain ${txMetadata.data[0].json_metadata.network} not supported or wrong/missing label 87 metadata`);
            errorService.internalError(res);
            res.status(200).send();
            return;
        }
      } else {
        console.log("Metadata label is not 674 for bridge event, skipping output notification received...");
        res.status(200).send();
        return;
      }

    } else {
      console.log("webhook type is not supported: ", webhookEvent.type);
      errorService.internalError(res);
      return;
    }
  } catch (error) {
    console.error("[aldeaBridgeEvent] ERROR:", error);
    console.error("[aldeaBridgeEvent] Error stack:", error.stack);
    errorService.internalError(res);
    return;
  }
});
...
const aldeaLockEvent = catchAsync(async (req, res) => {
  try {
    ...
    if (webhookEvent.type === "transaction") {
      ...
      console.log("Metadata: ", JSON.stringify(txMetadata.data, null, 2));

      if(txMetadata.data[0].label === '87') {
        // Validate tx metadata
        ...
        console.log("Metadata label is not 87 for lock event, skipping output notification received...");
        res.status(200).send();
        return;
      }
    } else {
    ...
    console.error("[aldeaLockEvent] ERROR:", error);
    console.error("[aldeaLockEvent] Error stack:", error.stack);
    errorService.internalError(res);
  }
});
